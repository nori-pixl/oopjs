/**
 * oop-lib (UMD build)
 * ビルドツール不要。<script src="oop-lib.js"></script> で読み込むだけで
 * グローバルに `OOP` オブジェクトが生成されます。
 * Node.js (CommonJS) でも require('./oop-lib.js') として利用可能です。
 *
 * このファイルは src/ 以下のソースを1ファイルにまとめた配布用ビルドです。
 */
(function (root, factory) {
  ...
  function createInterface(name, methodNames) {
    methodNames = methodNames || [];
    return Object.freeze({
      __isInterface: true,
      name: name,
      methods: methodNames.slice(),
    });
  }

  function implementsInterface(instance, iface) {
    if (!iface || !iface.__isInterface) {
      throw new TypeError('第2引数には createInterface() で作成したオブジェクトを渡してください。');
    }
    return iface.methods.every(function (methodName) {
      return typeof instance[methodName] === 'function';
    });
  }

  function enforceInterfaces() {
  ...
  function abstractClass(Base, abstractMethods) {
    Base = Base || Object;
    ...
    AbstractBase.__abstractMethods = abstractMethods.slice();
    return AbstractBase;
  }
  ...
  function createMixin(instanceMethods, staticMethods) {
  ...
  function mixClasses(Base, ...classes) {
    const Mixed = class extends Base {};
  ...
  class EventEmitter {
    ...
    emit(event, ...args) {
      const set = this.#listeners.get(event);
      if (set) set.forEach((handler) => handler.apply(this, args));
    }
  ...
  function observable(target, propertyName, initialValue) {
  ...
  function makeObservable(instance, properties) {
  ...
  function typed(paramTypes, returnType) {
  ...
  function typedMethod(paramTypes, returnType) {
  ...
  const oopjs = createOopjs();
  const oopjsOp = oopjs.op;
  const oopjsDuck = oopjs.duck;
